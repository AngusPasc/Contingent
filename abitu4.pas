...
procedure Tabitufsso.FormActivate(Sender: TObject);
begin

 datamodule1.baza_h.TableName :='d:\abitu.kol\baza_h.dbf';
 datamodule1.snspec.TableName :='d:\abitu.kol\snspec.dbf';
 ...
 filtr := '((otd=1) or (otd=2) ) and ((zach<>0) or (zach1<>0))';
 DataModule1.baza_h.Filter := filtr;
 DataModule1.baza_h.Filtered :=True;
end;

procedure Tabitufsso.FormClose(Sender: TObject; var Action: TCloseAction);
begin
...
procedure Tabitufsso.DBLookupComboBox1Click(Sender: TObject);
begin
  zach:= datamodule1.snspec.FieldByName('kod').AsInteger;
  otd := datamodule1.snspec.FieldByName('sprotd').AsInteger;
  if otd = 2 then otd:=3;   //заочное
  fak := 8;                // ФССО
  spec := 14;               // Правоведение в деканате

 Datamodule1.baza_h.Filtered := False;
 filtr := '((otd=1) or (otd=2)) and ((zach='+inttostr(zach)+') or (zach1='+inttostr(zach)+'))';
 Datamodule1.baza_h.Filter:= filtr;
 Datamodule1.baza_h.Filtered := True;
 Edit1.Text:=inttostr(Datamodule1.baza_h.RecordCount);
end;

procedure Tabitufsso.BitBtn2Click(Sender: TObject);
var
...
begin
 ...
 if nompr.Text='' then
  ...
  filtr := '((otd=1) or (otd=2)) and (zapis>0) and ((zach='+inttostr(zach)+') or (zach1='+inttostr(zach)+'))';
  DataModule1.baza_h.Filter := filtr;
  DataModule1.baza_h.Filtered :=True;
...
procedure Tabitufsso.BitBtn1Click(Sender: TObject);
begin
 close;
end;
