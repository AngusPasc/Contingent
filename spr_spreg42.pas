...
var
  spraion_up: Tspraion_up;
  reg :integer;
implementation
 uses main1,spr_spreg41;

{$R *.dfm}

procedure Tspraion_up.FormActivate(Sender: TObject);
begin
...
procedure Tspraion_up.FormClose(Sender: TObject; var Action: TCloseAction);
begin
 spraion.Enabled :=True;
end;

procedure Tspraion_up.BitBtn1Click(Sender: TObject);
var
 s : string;
 p: char;
begin
 if trim(Edit2.Text)='' then
  begin
   showmessage('Введите название района !');
   Edit2.SetFocus;
   exit;
  end;
 if reg=0 then
 ...
 if act =1 then begin
   s :='insert into geography (name,tip,rod,priz) values ('+''''+trim(Edit2.Text)+''''+',3,'+inttostr(reg)+','+''''+'1'+''''+')';
   ...
   s :='update geography set name='+''''+Edit2.Text+''''+',tip=3,rod='+inttostr(reg)+',priz='+''''+p+''''+'where pin='+inttostr(pin);;
   DataModule1.Query3.SQL.Add(s);
  end;
...
procedure Tspraion_up.BitBtn2Click(Sender: TObject);
begin
 close;
end;

procedure Tspraion_up.DBLookupComboBox1Click(Sender: TObject);
begin
